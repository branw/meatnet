...
func (bb *BitBuffer) ReadBits(bitLength uint64) (uint64, error) {
	...
	value := uint64(0)
	for i := uint64(0); i < min(bitLength, 64); i++ {
		bit := (bb.data[bb.bitOffset/8] >> (bb.bitOffset % 8)) & 1
		value |= uint64(bit) << i
		bb.bitOffset++
	}
	// We can't parse single values larger than 64 bits, so we can just skip
	// over any remaining bits
	if bitLength > 64 {
		bb.bitOffset += bitLength - 64
	}
	return value, nil
}
